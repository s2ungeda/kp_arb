...
- 에디트: 총 진입수량 / 1회주문수량 / 진입 기준값 / 청산 기준값. **진입 > 청산 필수**. 입력값은 저장(재시작 유지).
- 체크박스: 시작(자동 모드만 표시), LS/HL(체크된 거래소만 주문 — 단독 체크는 수동 전용 용도: 장시작·동시호가 급변, LS 시세 이상 시 사용자 판단 주문).
- 버튼: 진입주문/청산주문(수동만 표시), **pause**(진행 중 주문 포함 전량 취소), 옵션(재시도 설정 창).
- 모드별 표시: 수동 = 주문 버튼 표시 + 기준값 에디트 disable / 자동 = 시작 체크박스 표시 + 기준값 에디트 enable.
- 모니터링(1초): 현재진입수량(진입/청산 가능 수량), 패널1 = **진입**(빨강 바탕·흰 볼드), 패널2 = **청산**(파랑 바탕·흰 볼드) — 단 호가 대신 **estprice**로 계산(아래 3). (정정 2026-07-20: 순진입/순청산 아님)
