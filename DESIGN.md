...
- **역산 maker 주문가** (확정 2026-07-22): `국내 주문가 = 기준가 × (1 + HL괴리(est) − 기준값)` — 이 가격에 체결되면 스프레드 ≥ 기준값이 보장(기준가: SF=선물이론가, 주식=현재가). 틱 반올림(매수 내림/매도 올림, `ticks.py` — 표는 실측 확인 예정) 후 **maker 유지 보정**: 매수 주문가 ≥ 매도1호가면 매도1호가−1틱, 매도 주문가 ≤ 매수1호가면 매수1호가+1틱. 괴리보드에 HL est-pr·역산 주문가 표시(수량 세트·기준값 입력).
