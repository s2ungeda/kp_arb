...
### 6.2 전략 화면 + 주문 규칙 (사용자 확정 2026-07-20)
...
- 콤보1 종목: 하이닉스(기본)/삼성/현대차. 콤보2 상대: 주식/주식선물(기본). 콤보3 모드: 수동/자동T(동시 taker)/자동M(국내 maker→HL taker).
- **입력 세트 ×3** (추후 확장 가능): 총 진입수량 / 1회주문수량 / 진입 기준값 / 청산 기준값 + **세트별** 시작 체크박스(자동 모드만 표시)·진입/청산 주문 버튼(수동만 표시). **진입 > 청산 필수**. 입력값은 저장(재시작 유지).
- 체크박스: LS/HL(체크된 거래소만 주문 — 단독 체크는 수동 전용 용도: 장시작·동시호가 급변, LS 시세 이상 시 사용자 판단 주문).
- 정지/일시정지 (자동 모드 전용, 세트별 체크박스 — 수동엔 없음. 확정 2026-07-20): **시작 해제** = 자동 프로세스 중지 + 미체결 전량 취소. **PAUSE 체크** = 프로세스는 유지(신호 계산 계속)하되 미체결 취소 + 신규 발주 중단 — 해제 시 즉시 재개.
- 버튼: 옵션(설정 창 — 모달: 재시도 3종 + **주문 가능 시간**(시작~종료, 이 창 밖 시간엔 주문 금지) + **주식 신용거래 여부**).
- 모드별 표시(세트마다 적용): 수동 = 주문 버튼 표시 + 기준값 에디트 disable / 자동 = 시작 체크박스 표시 + 기준값 에디트 enable.
- 모니터링(1초): **전용 수량 에디트**(이 수량으로 estprice 계산 — 세트 입력과 별개), 패널1 = **진입**(빨강 바탕·흰 볼드), 패널2 = **청산**(파랑 바탕·흰 볼드) — estprice 기반(아래 3). 콤보로 선택한 **2종목(국내·HL) 현재가**, **주문 가능 시간**, 현재진입수량. (정정 2026-07-20: 패널은 순진입/순청산 아님)

**2) 수량 규칙**

- 1회주문수량은 **국내 기준**(콤보1·2 선택 기준). 환산: 주식 1:1, 주식선물 1계약:10주.
- 총 진입수량 = 국내 최대 보유 한도. 주식은 0~+한도(공매도 금지 §3), **주식선물은 양방향 한도**(|포지션| ≤ 총 진입수량 — 잔고 없어도 청산부터(=숏 진입) 가능).

**3) estprice (주문가격·신호 계산 공용)**

- 매수 estprice = 매도호가 잔량을 최우선부터 누적, **누적합 ≥ 주문수량이 되는 첫 매도호가**. 매도는 반대(매수호가 누적).
- 진입/청산·순진입/순청산 공식(§6.1)의 매수가/매도가 자리에 estprice를 사용 — "이 수량을 실제로 받아줄 가격" 기준 신호.
- 주문가격: 기준값 입력 → §6.1 공식 역산으로 주문가격 산출(estprice 기반). 호가 10단계 합계로도 부족하면 신호 없음(주문 금지).

**4) 주문·재시도**

- 진입 = LS 매수 + HL 매도 / 청산 = LS 매도 + HL 매수 (수동 버튼·자동 공통).
- 주문 전 검증: 입력값(수량·기준값) → 한도(2) → 거래소 체크 상태 → 세션 가드(아래 5).
- **한쪽 다리 실패/미체결 시 재시도** — 옵션 창에서 재시도 횟수/재시도 간격/미체결 대기 타이머 설정. 재시도 중 화면에 **양쪽 포지션 불일치 경고** 표시. 횟수 소진 후 자동 모드는 자동 주문 재개(잔여 불일치는 경고 유지 — 강제 해소 방법은 [OPEN]).

**5) 자동 주문 가드**

- 국내 **정규장에서만** 자동 주문. **사이드카·서킷브레이커 시 자동 중지**(JIF 장운영 코드 기반). 데드존(§7) 신규 진입 금지.
- 시작 해제 = 자동 정지 + 미체결 전량 취소 / PAUSE 체크 = 프로세스 유지·발주만 중단(미체결 취소).

**6) 기록·결산**

- 주문·체결·신호를 모두 기록(코어). 당일 장 마감 후 결산: 진입/청산 평균가, 총체결량 등.
