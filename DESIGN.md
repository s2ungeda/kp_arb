...
### 6.1 전략 v0.1 — 상대호가 괴리 (확정 2026-07-07, 원본: 사용자 엑셀 IM.xlsx 수식 이관)

**아이디어**: HL perp와 국내(주식선물·ETF)를 같은 잣대("주식 대비 괴리 %")로 정규화한 뒤,
**실제 체결 가능한 호가끼리의 괴리차(스프레드)** 가 벌어지면 양쪽에 반대 포지션으로 진입하고,
좁혀지면 청산한다. 펀딩(직전/예상)은 보유 방향 판단의 보조 지표.
...
**2) 진입/청산 신호** — 국내 상대(주식선물 또는 ETF)별로:

- 방향 A (국내 롱 + HL 숏): 진입 스프레드 = `HL 매수호가disp − 국내 매도호가disp` (HL bid에 팔고 국내 ask에 사는 taker 조합). 청산 = `HL 매도호가disp − 국내 매수호가disp`.
- 방향 B (국내 숏 + HL 롱): 부호 반대. **국내 신규 숏은 주식선물로만**(공매도 금지 §3 — ETF 매도는 보유 청산만).
