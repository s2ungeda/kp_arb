...
- **시세(구현 2026-08-07):** 원달러선물 **근·차근 월물 체결(FC0)** 을 stock_ws로 일괄 구독(`select_usd_futures_months`, t8426에서 근·차근 자동 선택). **최근월물만** 환율이론가에 먹이고(§6.1), **차근월물은 저장만**(`fx_futures_price[월물코드]`) — 헤지 월물 선택 대비. **호가 TR은 미실측이라 2단계**(주문 가격 산정 시 붙임).
