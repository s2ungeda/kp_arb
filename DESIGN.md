...
4. ~~HL perp 사양~~ **[라이브 확정 v6.10]** 빌더 dex = **`xyz`**(trade.xyz). 심볼(실측 — v0.4 예상과 다름): 삼성 **`xyz:SMSN`** / 하이닉스 **`xyz:SKHX`** / 현대차 **`xyz:HYUNDAI`**. szDecimals 3, 최대 **10x**, 펀딩 배수 0.5. **HIP-3는 dex별 마진 분리** — 조회·주문 모두 `dex="xyz"` 스코프, 자금은 메인 perp와 별도로 xyz 계정에 이체 필요. 에이전트 서명은 hyperliquid-python-sdk가 처리(`Exchange(wallet=에이전트, account_address=메인주소, perp_dexs=["xyz"])`). 시장가는 없음 → IOC 지정가(슬리피지 허용)로 대응. **주문 왕복(접수 oid→취소) 실계정 검증 완료.** 비밀: `HL_AGENT_KEY`(에이전트 키)·`HL_ACCOUNT_ADDRESS`(메인 주소).
