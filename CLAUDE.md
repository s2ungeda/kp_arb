...
## 1. 계약: DESIGN.md가 최우선
- `DESIGN.md`(및 그 안에서 링크한 `DESIGN-*.md`)가 이 프로젝트의 **단일 진실(spec/계약)** 이다. DESIGN.md는 제목·요약·링크만, 상세는 분할 파일에 둔다(예: 수동 주문 = `DESIGN-manual-order.md`). 작업 전 관련 분할 파일도 읽는다.
...
- **공매도 기본 금지(자동T 대주 예외):** 국내 주식(spot) 숏은 기본적으로 만들지 않는다(숏은 선물 매도 / 인버스 ETF). **단 자동T 역방향은 대차 공매도(대주)를 허용** — 설정창 주문유형 옵션으로만(2026-08-24, DESIGN §3 / DESIGN-auto-t.md).
- **FX 노출 헤지는 외부 위임:** 주 노출은 `fx.usd_exposure`로 계산해 **외부 #2로 보고만** 한다(주문 안 냄). **단, 별도 용도의 원달러선물 헤지 계좌 `Account.KR_FX`(DESIGN §9.1)는 예외** — 이 계좌로는 헤지 주문을 낼 수 있다(주문 흐름은 2단계 [OPEN], 아직 미구현).
...
- **.bat는 CRLF 줄바꿈 + 순수 ASCII로 저장** — Write 도구는 LF로 쓰므로, .bat 편집 후엔 PowerShell `Set-Content -Encoding ascii`(CRLF)로 재작성한다. LF나 비ASCII(예 em대시 '—')가 섞이면 cmd가 오파싱한다('m'은(는) 명령이 아닙니다 류).
- **tkinter 화면 스레드에서 네트워크 호출 금지** — 창 끌기·메뉴까지 통째로 얼어 버벅인다(메인 화면에서 실증). 폴링은 뒷단 스레드가 하고, 화면은 저장된 결과만 after()로 읽는다.
- **스펙이 원본(목업·이미지·델파이·엑셀·외부문서)을 가리키면 작업 전 그 원본을 먼저 연다** — "글이 우선"류 우선순위 규칙은 *그림을 본 뒤* 충돌을 판정하는 규칙이지, 안 봐도 된다는 뜻이 아니다(안 보면 무엇이 우선인지 따질 대상조차 없다). 특히 화면(UI)은 단위 테스트로 못 잡으니 목업을 반드시 열고, 바꾼 뒤엔 **띄워서 목업과 대조**한다. (실패 실증: order_hl 배치 — 목업 안 보고 글만 파싱해 배치가 어긋남)
- **UI·스펙 작업은 시작할 때 "근거"를 한 줄로 먼저 밝힌다** — 예: `근거: DESIGN §1 + order_hl_layout.png(열어봄) / 안 연 것: 델파이 원본`. 건너뛴 원본이 글로 보여야 사용자가 앞단에서 잡는다(침묵한 가정이 실수의 뿌리).
- **전략 이해·판단이 확실하지 않으면 추측으로 코드를 쓰지 말고 질문한다** — 미확정·모호한 요구를 임의로 메우지 말고 멈춰 묻는다(`[OPEN]`뿐 아니라 전략 로직·의도 전반). 잘못된 추측 위에 쌓은 코드가 가장 비싸다 — 지우고 다시 하는 값 + 사용자 신뢰.
