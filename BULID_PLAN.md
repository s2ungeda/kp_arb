...
> **다음 장중 리허설 절차**: main.bat → 코어 자동 시작(콘솔 "리허설 판정 + FX 보고 시작"
> 확인) → 자동M 주문 화면 → 1회수량(진입/청산)·설정(보유최대수량)·세트설정(기준값·목표량)
> 입력 → **운영시간을 지금 시각 포함으로** → 실행 체크 → 상단 진입/청산 신호 확인 →
> 기준값 넘으면 `logs/core_날짜.log`에 `[리허설 발주]` 찍힘(발주 없음, 안전).
> 확인할 것: ① 신호가 뜨는가(시세) ② 로그가 찍히는가(판정) ③ 값이 맞는가(계산).
>
> **새 스펙 대기 (밈전략_정리_20260730.xlsx, 2026-07-30)**: 자동주문 구조 전면 개편
> — 바로쏴(HP-S/HP-SF 동시 taker) + 체결쏴(HP-SF maker 5단계), 잔량비(min(1회수량,
> 상대1호가잔량×0.4)), 조회항목(avg·EN/EX SPRD·가중평균 환율), 일거래한도 2종, 리스크
> 가드 재도입(entry 0 이하/exit 0.5% 이상 불가 — **기존 '가드 폐기'와 충돌, 확정 필요**).
> **리허설로 판정 검증 후 DESIGN §6.2 전면 개정 → 화면·코어 재작업.**
...
## Phase 8 — 24시간 무중단 운영 (확정 2026-08-05)

> 목적: 무인 24시간 안전 운영. 아래는 사용자 확정 사항 = 계약. **전략과 무관한 항목부터** 구현.

### 결정 사항 (2026-08-05 사용자 확정)

- **주문 안전차단**: WS(시세·주문) 하나라도 끊기거나, 시세 WS가 N초 이상 무데이터면 주문 차단. N초는 전체환경설정.
- **WS 세션 현황 화면**(메인창): no·거래소·이름(시세/주문)·상태·수신카운트. 끊김/재접속 로그 남김.
- **재연결**: 자동 재접속 + 재연결 시 ①구독 전부 재설정(JIF 세션 포함) ②OrderBook 재스냅샷(포지션/미체결 재조회 — **주기 재동기 함수를 재연결 훅이 즉시 호출**해 겸함).
- **LS 토큰**: 매일 07:50 선갱신(장 시작 전). 실패 재시도·알림.
- **RateLimit**: 상한 근접 경고, 초과 시 거부(사유=RateLimit 초과)·경고.
...
- **부팅/전원**: 자동 재부팅 끄기 + 부팅 자동시작 + UPS(인프라, 코드 밖). 재부팅 후 **거래소 재조회로 포지션·잔고·미체결은 복구**되나, **로컬 카운터(발주누적 등)는 디스크 저장 필요** → 전략 재작업 시 상태파일로.
- 메인·코어 한 PC(서버 분기 후 재논의). 새벽은 국내장 없어 대부분 수동.

### 전체환경설정 항목 (신규 — 별도 설정 스펙으로 관리)

무데이터 N초 · 재동기 사용여부·주기(분) · 결산 시각 · 코어 자동재시작 사용여부·감지횟수 · 텔레그램 토큰/chat_id(비밀).

### 구현 순서 (전략 무관 → 먼저)

1. **알림 모듈(텔레그램)** — `kp_arb/alert.py`. 미설정 시 no-op, 전송 실패 삼킴(알림이 호출자를 죽이지 않음). ← 다른 기능이 호출하는 토대
2. **로그 자정 롤오버** — core_server `_setup_logging`.
3. **WS 세션 현황** — WS 계층이 연결상태·수신카운트 노출 → 메인창 표시.
4. **재연결 재구독 + 재동기 훅**.
5. **코어 자동 재시작** — 메인 감시 + 의도적종료 플래그 + 알림.
6. **주문 안전차단**(WS 끊김·스테일 게이트).

> 전략 결합 항목(카운터 리셋 시점·상태파일 저장/복원)은 **전략 재작업과 함께** — 지금 안 함.
