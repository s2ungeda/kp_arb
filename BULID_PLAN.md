...
### 7-1.5. 기준값 분석 (데이터 수집 중)

```
spread CSV 며칠치가 쌓이면: 순진입 분포(백분위) → 후보 기준값 격자 → 과거 재현
(회수·평균수익·보유시간·최대역행·미청산율) → 날짜 분리로 강건성 확인.
산출물: 쌍별 기준값 후보 표 — 최종 선택은 사용자.
```

### 7-2. 수동 모드

```
순진입이 config 기준값을 넘으면 신호 표시(모니터 강조 + 알림). 주문은 사람이.
신호 발생/해소를 파일로 기록(재현 검증용). 화면과 판단 로직은 분리(headless 대비).
완료 조건: 신호 판정 순수 로직 단위 테스트 통과 + 장중 신호 확인.
```

### 7-3. 자동 모드 (자동1 → 자동2 순)

```
자동1: 신호 시 양쪽 동시 taker. 자동2: 국내 maker(페깅 재사용) 체결 → HL taker.
수량·비율(환산주수)·손절·리스크 한도는 이 단계에서 확정(§13 #5). 모의(LS)+소액(HL)부터.
완료 조건: 전략 시그널·주문 흐름 단위 테스트 + 모의 왕복 실증.
[규칙]
```
